...
locals {
  ...
  read_storage_data_group_name = local.read_storage.resource_group_name
}
...
locals {
  ...
  write_storage_data_group_name = local.write_storage.resource_group_name
}
# Combine read and write storages into a map
locals {
  storages     = { for index, storage in distinct([local.read_storage, local.write_storage]) : index => storage }
  storages_map = { for storage in local.storages : storage.name => storage }
}
